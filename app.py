...
# ==============================
# STYLES CSS pour les statuts encadrés
# ==============================
st.markdown("""
    <style>
    .status-badge {
...
if "data_ready" in st.session_state and st.session_state["data_ready"]:
    ...
    # Interface pour changement de référence (avant le cercle)
    st.markdown("---")
    st.markdown("### 🔄 Gestion des changements de référence")
    st.markdown("Sélectionnez un article 'Missing' et un article 'Packing only' pour les fusionner comme changement de référence")
    ...
    if st.session_state.get("ref_changes"):
        st.info("**Changements de référence actifs :**")
        for old, new in st.session_state["ref_changes"].items():
            st.write(f"`{old}` → `{new}`")
        if st.button("🗑️ Réinitialiser tous les changements"):
            st.session_state["ref_changes"] = {}
            st.rerun()
    ...
    # 3. TABLEAU DES RÉSULTATS (après le cercle)
